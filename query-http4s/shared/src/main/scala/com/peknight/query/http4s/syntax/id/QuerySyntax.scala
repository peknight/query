...
trait QuerySyntax:
  extension [A] (a: A)
    def toQuery(using Encoder[Id, com.peknight.query.Query, A], Config): Query = QueryOps.toQuery[Id, A](a)
  end extension

  extension (query: Query)
    def parse[A](using Decoder[Id, Cursor[com.peknight.query.Query], A]): Either[Error, A] =
      QueryOps.parseQuery[Id, A](query)
    def withQuery[A](a: A)(using Encoder[Id, com.peknight.query.Query, A], Config): Query =
