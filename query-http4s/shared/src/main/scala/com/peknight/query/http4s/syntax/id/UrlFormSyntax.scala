...
trait UrlFormSyntax:
  extension [A] (a: A)
    def toUrlForm(using Encoder[Id, Query, A], Config[String]): UrlForm = UrlFormOps.toUrlForm[Id, A](a)
  end extension

  extension (urlForm: UrlForm)
    def parse[A](using Decoder[Id, Cursor[Query], A]): Either[Error, A] = UrlFormOps.parseUrlForm[Id, A](urlForm)
    def withQuery[A](a: A)(using Encoder[Id, Query, A], Config[String]): UrlForm =
