...
object UrlFormOps:
  def toUrlForm[F[_], A](a: A)(using Functor[F], Encoder[F, Query, A], Config[String]): F[UrlForm] =
    a.toMap[F, String].map(UrlForm.apply)
  ...
  def withQueryParams[F[_], A](urlForm: UrlForm, a: A)(using Functor[F], Encoder[F, Query, A], Config[String]): F[UrlForm] =
    a.toMap[F, String].map(m => UrlForm(urlForm.values ++ m))
end UrlFormOps
