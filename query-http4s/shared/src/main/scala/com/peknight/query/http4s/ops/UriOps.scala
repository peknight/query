...
object UriOps:
  ...
  def withQueryParams[F[_], A](uri: Uri, a: A)(using Functor[F], Encoder[F, A], Config): F[Uri] =
  ...
  def withFragmentParams[F[_], A](uri: Uri, a: A)(using Applicative[F], Encoder[F, A], Config)
