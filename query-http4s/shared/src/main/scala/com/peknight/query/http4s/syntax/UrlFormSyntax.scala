...
trait UrlFormSyntax:
  extension [A] (a: A)
    def toUrlForm[F[_]](using Functor[F], Encoder[F, Query, A], Config[String]): F[UrlForm] = UrlFormOps.toUrlForm[F, A](a)
  end extension
  ...
  extension (urlForm: UrlForm)
    ...
    def withQuery[F[_], A](a: A)(using Functor[F], Encoder[F, Query, A], Config[String]): F[UrlForm] =
