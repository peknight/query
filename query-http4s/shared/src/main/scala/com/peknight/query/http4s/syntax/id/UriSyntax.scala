...
trait UriSyntax:
  extension (uri: Uri)
    ...
    def withQuery[A](a: A)(using Encoder[Id, Query, A], Config): Uri = UriOps.withQueryParams[Id, A](uri, a)
    def parseFragment[A](using Decoder[Id, Cursor[Query], A]): Either[Error, Option[A]] =
      UriOps.parseFragment[Id, A](uri)
    def withFragmentParams[A](a: A)(using Encoder[Id, Query, A], Config): Either[ParsingFailure, Uri] =
