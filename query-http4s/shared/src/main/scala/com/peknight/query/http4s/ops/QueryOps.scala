...
object QueryOps:
  def toQuery[F[_], A](a: A)(using Functor[F], Encoder[F, com.peknight.query.Query, A], Config[String]): F[Query] =
    a.pairs[F, String].map(p => Query.fromVector(p.toVector))
  ...
  def withQueryParams[F[_], A](query: Query, a: A)(using Functor[F], Encoder[F, com.peknight.query.Query, A], Config[String])
  : F[Query] =
    a.pairs[F, String].map(p => query ++ p.toVector)
end QueryOps
