...
trait QuerySyntax:
  ...
  extension (query: Query)
    def to[F[_], A](using Monad[F], Decoder[F, A]): F[Either[Error, A]] = QueryOps.fromQuery[F, A](query)
    def ++[F[_], A](a: A)(using Functor[F], Encoder[F, A], Configuration): F[Query] = QueryOps.queryWith[F, A](query, a)
  end extension
end QuerySyntax
