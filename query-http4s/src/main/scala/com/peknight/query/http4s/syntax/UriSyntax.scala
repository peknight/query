...
trait UriSyntax:
  extension (uri: Uri)
    ...
    def withQuery[F[_], A](a: A)(using Functor[F], Encoder[F, Query, A], Config[String]): F[Uri] =
    ...
    def withFragmentParams[F[_], A](a: A)(using Applicative[F], Encoder[F, Query, A], Config[String])
